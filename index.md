...
non-linear, evaluating it on daily summaries of each weather variable is
not the same as evaluating it hourly and then summarising the result —
and it is much less useful. Daily maxima of temperature and of humidity
typically occur many hours apart, so an index built from them describes
